...
class InvitesForm(npyscreen.FormBaseNew):
    def create(self):
    ...
    def toggle(self, *args, **kwargs):
        if self.status.value == "ON":
            ...
            self.parentApp.main_form.invites_title.value = "OFF"
            self.control_external.invites_status = "OFF"
        elif self.status.value == "OFF":
            self.status.value = "ON"
            self.parentApp.main_form.invites_title.value = "ON"
            self.control_external.invites_status = "ON"
        self.status.display()

    def switch_screen_EDIT_LINKS(self, *args, **kwargs):
        self.parentApp.change_form("EDIT_INVITES")
