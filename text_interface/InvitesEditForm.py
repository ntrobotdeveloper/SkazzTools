...
class InvitesEditButton(npyscreen.Button):
    def __init__(self, parent, data=None, source=None, out=None, *args, ** keywords):
        super(InvitesEditButton, self).__init__(parent, *args, ** keywords)
        self.data = data
        self.source = source
        self.out = out
        ...
        if self.source:
            if self.value:
                if self.source.name == "Add one":
                    with open("program_files/invites.txt", "r+") as f:
...
class InvitesEditForm(npyscreen.FormBaseNew):
    def create(self):
        y, x = self.useable_space()
        try:
            with open("program_files/invites.txt", "r") as f:
                file_data = f.readlines()
                self.invite_list = [str(i)+'. ' + x for i, x in enumerate(file_data)]
        except FileNotFoundError:
            with open("program_files/invites.txt", 'w'):
                self.invite_list = []
        ...
        self.button_obj = self.add(InvitesEditButton, name="Accept", rely=20, data=self.invite_list,
    ...
    def show_list(self, *args, **kwargs):
        if self.outputbox_obj.hidden:
            ...
            self.outputbox_obj.update_value(self.invite_list)
        else:
            self.outputbox_obj.hidden = True
            self.outputbox_obj.update_value(self.invite_list)
